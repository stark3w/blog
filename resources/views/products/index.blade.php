            ...
            <div class="col-md-9">
                <div class="row row-cols-1 row-cols-md-3 g-4">
                    @foreach($products as $product)
                        <div class="col">
                            <a href="{{ route('products.show',  ['catalog_slug' => $catalog->slug,'product_slug' => $product->slug, ]) }}" class="text-decoration-none text-dark">
                                <div class="card h-100 d-flex flex-column">
                                    ...
                                    <div class=" mt-auto">
                                        ...
                                        @can('delete',\App\Models\Product::class)
                                            <form action="{{ route('products.destroy', $product->id) }}" method="post" onsubmit="return confirm('Точно хотите удалить товар?')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-outline-danger w-100 mt-2">Удалить товар</button>
                                            </form>
                                        @endcan
                                    </div>
                                </div>
